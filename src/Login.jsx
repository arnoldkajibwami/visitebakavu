...
function App() {
    ...
    const handleSubmit = (event) => {
        ...
        var { uname, pass } = document.forms[0]; // Find user login info 
        const userData = database.find((user) => user.username === uname.value); // Compare user info 
        if (userData) {
            if (userData.password !== pass.value) { // Invalid password 
    ...
    return (
        <div className="app">
            <div className="login-form">
                ...
                {isSubmitted ? <div>User is successfully logged in</div> : renderForm} </div>
        </div>
